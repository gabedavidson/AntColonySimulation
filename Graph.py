...
class Graph:
    ...
    def randomize(self, n, w, h, c_hint: float = .2, connect: bool = False, max_connect_attempts_ratio: int = 1):
        assert n > 2
        coordinates = {}

        # setup nodes in space randomly
        for i in range(n):
            self.chart[i] = {}
            coordinates[i] = [round(random.random() * w), round(random.random() * h)]
        ...
        if connect:  # fixme. not working when connected is True
            self.connect(n, c_hint)
    ...
    def connect(self, n, c_hint: float = 0.2):
        # assigning edges
        for n1 in range(n - 1, -1, -1):
            ...
            for k in range(n1):
                ...
                if random.random() <= c_hint:  # as c_hint -> 1, number of edges decreases
                    print(f'removed path from {n1} to {k}')
                    del self.chart[n1][k]
...
graph = Graph()
graph.randomize(n=10, w=15, h=15, c_hint=0.5, connect=True)
graph.print()
